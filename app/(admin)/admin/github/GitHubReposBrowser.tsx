...
export function GitHubReposBrowser({
	...
	const [reviewProjects, setReviewProjects] = useState<ReviewProject[] | null>(
		null,
	);
	...
	function handleImport() {
		...
		startImporting(async () => {
			try {
				...
				toast.success(
					`${reviewProjects.length} project(s) imported successfully`,
				);
					...
					>
						...
						<div className="grid grid-cols-2 gap-4">
							<div>
								...
								<input
									...
									value={proj.title}
									onChange={(e) =>
										updateReviewField(i, "title", e.target.value)
									}
								/>
							</div>
							<div>
								...
								<input
									...
									onChange={(e) =>
										updateReviewField(i, "imagePath", e.target.value)
									}
								/>
							</div>
						</div>
						...
						<div>
							...
							<textarea
								...
								onChange={(e) =>
									updateReviewField(i, "description", e.target.value)
								}
							/>
						</div>
						...
						<div className="flex gap-4">
							<label className="flex items-center gap-2 text-sm text-zinc-400 cursor-pointer">
								<input
									...
									onChange={(e) =>
										updateReviewField(i, "isPrivate", e.target.checked)
									}
